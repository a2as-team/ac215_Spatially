...
class MunicodeScraper:
    ...
    def _close_download_panel(self):
        ...
        try:
            ...
            close_button = self.selenium_util.driver.find_element(By.XPATH, "//div[contains(@class, 'offcanvas-pane')]//button[@data-close]")
    ...
    def _open_download_panel(self):
        ...
        download_button = self.selenium_util.driver.find_element(By.XPATH, "//button[.//span[contains(text(), 'Download (Docx)')]]")
    ...
    def _expand_one_level(self):
        ...
        try:
            offcanvas = self.selenium_util.find_element(By.CSS_SELECTOR, ".offcanvas-pane.active")
            ...
            for li in root_li_elements:
                ...
                try:
                    expander = li.find_element(By.XPATH, "./button[contains(@class, 'expToc-expander')]")
    ...
    def _expand_specific_parent(self, parent_node_id):
        ...
        try:
            ...
            parent_li = self.selenium_util.find_element(By.CSS_SELECTOR, f"li[data-nodeid='{parent_node_id}']", timeout=5)

            # Find the expander button for this parent
            try:
                expander = parent_li.find_element(By.XPATH, "./button[contains(@class, 'expToc-expander')]")
                expander.click()
                time.sleep(0.3)
    ...
    def _collect_sections_max_1_level(self):
        ...
        try:
            ...
            for li in root_nodes:
                try:
                    # Get the heading
                    checkbox = li.find_element(By.CSS_SELECTOR, "button.expToc-selector[role='checkbox']")
                    heading_elem = checkbox.find_element(By.CSS_SELECTOR, "span[data-ng-bind]")
                    ...
                    try:
                        child_ul = li.find_element(By.XPATH, f".//ul[@id='child-nodes-{parent_node_id}']")
                        child_nodes = child_ul.find_elements(By.XPATH, "./li[@data-nodeid]")

                        # Collect ALL children at level 1 (regardless of whether they have sub-children)
                        for child_li in child_nodes:
                            try:
                                child_checkbox = child_li.find_element(By.CSS_SELECTOR, "button.expToc-selector[role='checkbox']")
                                child_heading_elem = child_checkbox.find_element(By.CSS_SELECTOR, "span[data-ng-bind]")
    ...
    def _retry_failed_downloads(self, failed_sections, downloaded_files):
        ...
        for idx, (path, parent_node_id, node_id, filename) in enumerate(failed_sections, 1):
        ...
        try:
            ...
            for idx, (path, parent_node_id, node_id) in enumerate(all_sections, 1):
                ...
                try:
                    ...
                    failed_sections.append((path, parent_node_id, node_id, safe_filename))
            ...
            if failed_sections:
                still_failed = self._retry_failed_downloads(failed_sections, downloaded_files)

                if still_failed:
                    print(f"\n{'='*60}")
                    print(f"Warning: {len(still_failed)} sections still failed after retry:")
    ...
    def _wait_for_download_complete(self, expected_extension='.docx', timeout=60, pre_existing_files=None):
        """
        ...
        while time.time() < end_time:
            ...
            if check_count % 10 == 0:
                ...
                print(f"[{elapsed}s] Checking... All files: {len(all_files)}, New: {len(new_files)}, Complete new: {len(complete_new_files)}, Incomplete: {len(incomplete_files)}")
    ...
    def _download_single_section(self, node_id, filename):
        ...
        download_dir = Path(self.download_dir)
        pre_existing_files = set(download_dir.glob("*.docx")) if download_dir.exists() else set()
        print(f"Starting download for node_id: {node_id}")

        # Find the checkbox fresh by node_id to avoid stale element
        try:
            li_element = self.selenium_util.find_element(By.CSS_SELECTOR, f"li[data-nodeid='{node_id}']", timeout=10)
            print(f"Found element with node_id: {node_id}")
        except Exception as e:
            print(f"ERROR: Could not find element with node_id '{node_id}': {e}")
            raise

        checkbox = li_element.find_element(By.CSS_SELECTOR, "button.expToc-selector[role='checkbox']")
        checkbox_text = checkbox.find_element(By.CSS_SELECTOR, "span[data-ng-bind]").text
